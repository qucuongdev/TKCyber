      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-end mb-12">
          <div>
            ...
            <p className="text-indigo-300/60">Latest writes from the SOC.</p>
          </div>
          <a href="#" className="hidden md:flex items-center gap-2 text-indigo-300 hover:text-white transition-colors font-mono text-sm uppercase">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {LATEST_POSTS.map((post) => (
            <article key={post.id} className="flex flex-col bg-sec-card/40 backdrop-blur-sm border border-indigo-800/50 rounded-lg overflow-hidden hover:border-indigo-400/50 transition-all duration-300 group hover:shadow-lg hover:shadow-indigo-500/10">
              <div className="p-6 flex flex-col h-full">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-indigo-900/50 mt-auto">
                   <span className="text-indigo-400/60 text-xs font-mono">{post.date}</span>
